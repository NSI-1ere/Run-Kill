...
class Life():
    def __init__(self, player):
        ...
        empty_heart_count = 3 - player.hp_counter
        ...
        empty_heart_image = self.loader.load_image(self.const.chemin_repertoire + r'.\Sprites\Life\EmptyHeart.png', self.const.heart_width, self.const.heart_height)
        ...
        for _ in range(empty_heart_count):
            empty_heart = LifeSprite(empty_heart_image, draw_pos, self.const.heart_y)
            player.all_hearts.add(empty_heart)
            draw_pos += self.const.heart_width + self.const.heart_offset
